...
if __name__ == "__main__":
    ...
    index_df = pd.read_csv("2017_2018.csv")
    save_name = "prompt3_data4_2017_2018"
                ...
                if current_bt_idx < len(bt_df):
                    ...
                    try:
                        ...
                        if cropped_arr.shape == (63, 100, 100):
                            ...
                            cropped_arr = torch.tensor(cropped_arr,).to(device).unsqueeze(0).float().to(device)
                            bt_his = torch.tensor(bt_his).unsqueeze(0).float().to(device)
                            
                            # breakpoint
                            x = [cropped_arr, bt_his]

                            output = train_model(cropped_arr)

                            out = output.detach().cpu().numpy()[0][0]
                            list_his.append(out)
